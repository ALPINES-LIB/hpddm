...
static PetscErrorCode PCSetFromOptions_HPDDM(PetscOptionItems *PetscOptionsObject, PC pc)
{
  ...
  if (i > 1) {
    ...
    if (flg) {
      ...
      if (!flg) SETERRQ(PETSC_COMM_SELF, PETSC_ERR_ARG_WRONG, "-%smat_mumps_use_omp_threads and -%spc_factor_mat_solver_type != %s", prefix, prefix, MATSOLVERMUMPS); // LCOV_EXCL_LINE
      size = n;
      n = -1;
      ierr = PetscOptionsGetInt(NULL, prefix, "-mat_mumps_use_omp_threads", &n, NULL);CHKERRQ(ierr);
      if (n < 1) SETERRQ(PETSC_COMM_SELF, PETSC_ERR_ARG_WRONG, "Need to specify a positive integer for -%smat_mumps_use_omp_threads", prefix); // LCOV_EXCL_LINE
      if (n * size > previous) SETERRQ(PETSC_COMM_SELF, PETSC_ERR_ARG_WRONG, "%d MPI process%s x %d OpenMP thread%s greater than %d available MPI process%s for the coarsest operator", (int)size, size > 1 ? "es" : "", (int)n, n > 1 ? "s" : "", (int)previous, previous > 1 ? "es" : ""); // LCOV_EXCL_LINE
    }
#endif
...
static PetscErrorCode PCHPDDMCreateSubMatrices_Private(Mat mat, PetscInt n, const IS*, const IS*, MatReuse scall, Mat *submat[])
{
  ...
  PetscFunctionBegin;
  if (n != 1) SETERRQ(PETSC_COMM_SELF, PETSC_ERR_ARG_WRONG, "MatCreateSubMatrices() called to extract %D submatrices, which is different than 1", n); // LCOV_EXCL_LINE
    ...
    if (data->N > 1 && (data->aux || ismatis || algebraic)) {
      ...
      if (!ismatis && sub == &data->aux && !data->B && subdomains && data->share) {
        ...
        if (size != 1) {
          ...
          } else SETERRQ(PETSC_COMM_SELF, PETSC_ERR_PLIB, "Number of subdomain solver %D != 1", size); // LCOV_EXCL_LINE
        } else {
          Mat        D;
          const char *matpre;
          PetscBool  cmp[2];
          ...
          if (!cmp[0] != !cmp[1]) SETERRQ(PETSC_COMM_SELF, PETSC_ERR_ARG_INCOMP, "-pc_hpddm_levels_1_pc_asm_sub_mat_type %s and auxiliary Mat of type %s",((PetscObject)D)->type_name,((PetscObject)C)->type_name); // LCOV_EXCL_LINE
          if (!cmp[0]) {
...
static PetscErrorCode PCHPDDMSetCoarseCorrectionType_HPDDM(PC pc, PCHPDDMCoarseCorrectionType type)
{
  ...
  PetscFunctionBegin;
  if (type < 0 || type > 2) SETERRQ(PETSC_COMM_SELF, PETSC_ERR_ARG_UNKNOWN_TYPE, "Unknown PCHPDDMCoarseCorrectionType %d", type); // LCOV_EXCL_LINE
